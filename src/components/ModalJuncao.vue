...
// LOGICA CORRIGIDA: Agrupa parcelas garantindo que o prazo apareça
const parcelamentoDetalhado = computed(() => {
  const grupos = {}
  props.cartas.forEach(c => {
    // Certifique-se de que o campo se chama 'prazo'. Se for outro nome no banco, mude aqui.
    const prazoEfetivo = c.prazo || 0 
...
<style scoped>
/* Mantive os estilos anteriores para manter o padrão visual */
.modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.4); display: flex; justify-content: center; align-items: center; z-index: 10000; }
.modal-juncao { background: white; width: 100%; max-width: 450px; border-radius: 12px; overflow: hidden; }
.modal-header { padding: 16px 20px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #f3f4f6; }
.modal-header h2 { color: #1e3a8a; font-size: 1.25rem; margin: 0; }
.btn-close { background: none; border: none; font-size: 1.2rem; cursor: pointer; color: #9ca3af; }
.modal-body { padding: 20px; }
.section-divider { height: 1px; background: #f3f4f6; margin: 16px 0; }
.parcelamento-box { background: #f9fafb; padding: 15px; border-radius: 8px; border: 1px solid #f3f4f6; }
.section-label { color: #6b7280; font-size: 0.85rem; font-weight: 700; text-transform: uppercase; margin-bottom: 10px; }
.parcela-item { display: flex; justify-content: space-between; margin-bottom: 5px; font-size: 1rem; color: #111827; }
.cota-badge { background: #e5e7eb; color: #4b5563; font-size: 0.7rem; padding: 2px 6px; border-radius: 4px; }
.parcela-total { margin-top: 10px; padding-top: 8px; border-top: 1px dashed #d1d5db; font-weight: 700; color: #1e3a8a; }
.modal-footer { padding: 15px 20px; background: #f9fafb; }
.btn-negotiate { width: 100%; padding: 12px; border-radius: 8px; background: #00b37e; color: white; font-weight: 700; border: none; cursor: pointer; }
</style>