...
// Lógica para agrupar e separar parcelas diferentes
...
const copiarTudo = async () => {
  ...
  texto += `DETALHAMENTO DAS PARCELAS:\n`
  parcelasAgrupadas.value.forEach((p) => {
    texto += `- ${p.prazo}x de ${formatCurrency(p.valor)}${p.qtdCotas > 1 ? ` (${p.qtdCotas} cotasneste valor)` : ''}\n`
  })
        ...
        <div class="modal-body">
          <div class="main-info">
            ...
            <div class="parcelamento-box">
              ...
              <div v-for="(p, index) in parcelasAgrupadas" :key="index">
                <div class="parcela-item-row">
                  <strong>{{ p.prazo }}x de {{ formatCurrency(p.valor) }}</strong>
                  <span v-if="p.qtdCotas > 1" class="cota-badge">{{ p.qtdCotas }} cotas</span>
                </div>
                <div v-if="index < parcelasAgrupadas.length - 1" class="operador-soma">+</div>
              </div>

              <div class="total-parcelas-footer">
                <p class="parcela-val">
                  Soma das parcelas: <strong>{{ formatCurrency(totalParcelaMensal) }}</strong>
                </p>
              </div>
            </div>
            ...
            <div class="cotas-selecionadas">
              ...
              <div v-for="c in cartas" :key="c.codigo" class="cota-item">
                Cód: {{ c.codigo }} - {{ c.tipo }} - {{ c.administradora_detalhes?.nome }}:
...
.operador-soma {
  text-align: center;
  font-size: 1.2rem;
  color: #94a3b8; /* Um tom de cinza suave */
  margin: 4px 0;
  font-weight: bold;
}
